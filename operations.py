...
OPS = {
    ...
    'se_layer': lambda C, stride, affine: SELayer(C, affine),
...
class SELayer(nn.Module):
    def __init__(self, channel, affine, reduction=16):
        super(SELayer, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        squeezed_channels = max(channel // reduction, 1)
        self.fc = nn.Sequential(
            nn.Linear(channel, squeezed_channels, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(squeezed_channels, channel, bias=False),
            nn.Sigmoid()
        )
