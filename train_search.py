...
#         pin_memory=True)
    ...
    for epoch in range(args.epochs):
        ...
        # training
        train_acc, train_obj, net = train(train_queue, valid_queue, model_adv, architect, criterion, optimizer, lr,
                                         perturb_alpha, epsilon_alpha, model2, epoch, optimizer2)
        ...
        valid_acc, valid_obj = infer(valid_queue, net, criterion)
        torch.save(net.state_dict(), "audience_{}.pth".format(epoch))
...
def train(train_queue, valid_queue, model, architect, criterion, optimizer, lr, perturb_alpha, epsilon_alpha, model2, epoch, optimizer2):
...
#         optimizer.zero_grad()
        ...
        #print (logits.size(), diff.size(), x.size())
...
#         logits2 = model2(x)
        ...
        total += target.size(0)
        #print (predicted, target, logits2.size())
    ...
    return  100.*correct/total, train_loss/(max_step+1), model2
